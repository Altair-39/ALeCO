...
void UpdateDrawFrame(void) {
  ...
  if (solutionFound) {
    ...
    DrawText(TextFormat("Attempts: %d", tentativi), margin, 50, 20, DARKBLUE);
  } else {
    DrawText("Press SPACE to find solution", margin, 20, 24, DARKBLUE);
  }
...
bool rifiuta(int regine[], int j) {
  ...
  for (int i = 0; i < j - 1 && !rifiuta; i++) {
    rifiuta =
        regine[j - 1] == regine[i] ||                       // same column?
...
void findSolution(void) {
  ...
  printf("Finding solution for 8 queens problem...\n");
  solutionFound = soluzione(queens, 0, boardSize);
...
int main(void) {
  ...
  while (!WindowShouldClose()) {
    // Check for space key to find solution
