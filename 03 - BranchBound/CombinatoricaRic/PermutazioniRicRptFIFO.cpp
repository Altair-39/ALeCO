...
class PermutazioniRicRptFIFO {
public:
  void risposte(list<ArrIntInt> &spazioStati, vector<int> a) {
    ArrIntInt node = eNode(spazioStati);

      if (!completo(node, a)) {
        if (!rifiuta()) {
            espande(node, spazioStati, a);
          while (!spazioStati.empty()) {
            risposte(spazioStati, a);
          }
        } else {
...
private:
   ...
   * appunto, completa.                                        */
  bool completo(const ArrIntInt &eNode, vector<int> a) {
    int j = eNode.pi1();
    return a.size() == j;
  }
...
void espande(const ArrIntInt& eNode, list<ArrIntInt>& spazioStati, vector<int> a) {
    ...
    while (i < a.size()) {
        ...
        spazioStati.push_back(nuovoLiveNode);
...
void testCase1() {
  ...
  PermutazioniRicRptFIFO generator;
  generator.risposte(liveNodes, initialSolution);
  cout << endl;
}
