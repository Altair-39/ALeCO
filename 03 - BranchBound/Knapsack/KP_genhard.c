...
long long generator(int n, int *pp, int *ww, int type, int r, int v,
                    int tests) {
  ...
  for (i = 0; i < span; i++) {
    sw[i] = randm(r) + 1;
    if (type == 11)
      sp[i] = randm(r) + 1; /* uncorr */
    if (type == 12)
      sp[i] = randm(2 * r1 + 1) + sw[i] - r1; /* wekcorr */
    if (type == 13)
      sp[i] = sw[i] + r1; /* strcorr */
    if (sp[i] <= 0)
      sp[i] = 1;
  ...
  for (i = 0; i < n;) {
    ...
    switch (type) {
    case 1:
      p = randm(r) + 1; /* uncorrelated */
      break;
    case 2:
      p = randm(2 * r1 + 1) + w - r1; /* weakly corr */
      if (p <= 0)
        p = 1;
      break;
    case 3:
      p = w + r1; /* strongly corr */
      break;
    case 4:
      p = w; /* inverse strongly corr */
      w = p + r1;
      break;
    case 5:
      p = w + r1 + randm(2 * r / 1000 + 1) - r / 1000; /* alm str.corr */
      break;
    case 6:
      p = w; /* subset sum */
      break;
    case 7:
      ...
      break;
    case 8:
      w = 2 * ((w + 1) / 2); /* even-odd knapsack */
      p = w + r1;
      break;
    case 9:
      p = w; /* uncorrelated, similar weights */
      w = randm(r1) + 100 * r;
      break;
    ...
    case 15:
      w = randm(r) + 1;
      p = 3 * ((w + 2) / 3); /* even-odd like profits */
      break;
    case 16:
      w = randm(r) + 1;
      p = 2 * sqrt(4 * r * r - (w - 2 * r) * (w - 2 * r)) / 3;
      break;

    default:
      perror("undefined problem type");
    }
    pp[i] = p;
    ww[i] = w;
    i++;
  ...
  default:
    perror("undefined capacity type");
  }
}
